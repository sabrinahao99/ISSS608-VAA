...
high_gpa <- final_ggplot$GPA_Label %in% c("3.2–3.6 (Good)", ">=3.6 (Excl)")
...
To compare students’ time-use distributions across GPA groups, we applied the Kruskal–Wallis (non-parametric) test. All three time variables were statistically significant across GPA categories.

• Study time: Median study time was 9 hours for every GPA group, mainly because many students fall into the ≥8 hours band. The clearest difference is that the lower-GPA group has a higher share of students with shorter study time.

• Social media time: The lower-GPA group shows a higher median social media time, suggesting they spend more time on social media.
