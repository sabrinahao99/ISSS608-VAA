...
## 2.9.1 Cycle Plot of Monthly Trade By cluster

First, I used a line chart of Monthly Trade Pattern by Cluster to compare the changes in trade patterns of different clusters throughout the year.
...
```{r}
#every cluster, average reference line for each month
hline_data <- cluster_year_month %>%
  group_by(cluster, month) %>%
  summarise(
    avgvalue = mean(avg_trade, na.rm = TRUE),
...
ggplot() +
  ...
  theme_minimal()
```
## 2.9.2 Monthly Balance Pattern by Cluster
...
## 2.9.4 Monthly Summary Table by Cluster
...
Kam, T.S. (2025).R for visual Analytics
