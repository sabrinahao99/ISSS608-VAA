...
-   Average Trade by Cluster

-   Growth Ratio by Cluster
...
knitr::kable(
  summary_table,
  caption = "Summary Table with Cluster Labels"
)
```
...
Users can adjust the "Number of countries" and "Number of clusters" parameters in the left-hand control panel. When these two parameters change, all charts on this page will update accordingly.
...
-   Statistical Comparison: Shows the statistical differences between clusters across two core metrics.
...
Users only need to uniformly set the year range, number of clusters, and Numbers of countries on the left, and all charts on the page will update simultaneously.
...
Users can continue to use the “Numbers of Countries" and "Number of Clusters" parameters on the left, and can also adjust the bubble encoding to see the display effect when different variables are used as bubble sizes.
